...
from config import Config, _INSECURE_SECRET_KEYS
...
def create_app():
    ...
    def _ensure_user_columns_sqlite() -> None:
        """开发兜底：users.phone（由 nickname 迁移或新建）。"""
        if not app.config.get("SQLALCHEMY_DATABASE_URI", "").startswith("sqlite"):
            return
        with db.engine.begin() as conn:
    ...
    def role_dashboard_endpoint(role: str) -> str:
        """按角色映射默认仪表盘 endpoint，未知角色回落到客户端。"""
        if role == "admin":
    ...
    _env = str(app.config.get("ENV") or os.getenv("FLASK_ENV", "") or "").lower()
    if _env == "production":
        sk = app.config.get("SECRET_KEY")
        if sk is None or str(sk).strip() == "" or sk in _INSECURE_SECRET_KEYS:
            raise RuntimeError(
                "生产环境必须在环境中设置强随机 SECRET_KEY（例如 openssl rand -hex 32），"
                "且不得使用仓库默认值或常见占位符。当前 FLASK_ENV=production。"
            )
        if len(str(sk)) < 32:
            raise RuntimeError(
                "生产环境 SECRET_KEY 长度过短（建议至少 32 字符）。请使用强随机串。"
            )
