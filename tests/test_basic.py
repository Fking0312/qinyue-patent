...
from app.workflow import TaskPhase, backfill_completed_case_actual_return_at, set_actual_return_at_from_latest_approved_writing
...
def test_homepage():
    ...
    assert "进入登录".encode("utf-8") in response.data
    assert "让每一个案件进度".encode("utf-8") in response.data
    assert b"home-hero" not in response.data
...
def test_admin_accounts_can_create_staff_and_client():
    ...
    with app.app_context():
        ...
        assert created.staff_function == "writer"
        assert created.staff_function_label == "撰写师"
    ...
    with app.app_context():
        created_client = User.query.filter_by(username=client_name).one()
        assert created_client.staff_function is None
        assert created_client.staff_function_normalized == ""
...
def test_deactivate_staff_removes_from_assignment_pool_and_unassigns():
    ...
    assert "员工账号已离职".encode("utf-8") in r_login.data


def test_deactivate_staff_keeps_assignee_for_non_in_progress_cases():
    app = create_app()
    suffix = uuid4().hex[:8]
    with app.app_context():
        admin = User(username=f"_admin_deact2_{suffix}", role="admin")
        admin.set_password("secret")
        staff = User(username=f"_staff_deact2_{suffix}", role="staff")
        staff.set_password("secret")
        db.session.add_all([admin, staff])
        db.session.flush()
        customer = Customer(kind=CustomerKind.COMPANY, name=f"_deact2_c_{suffix}")
        db.session.add(customer)
        db.session.flush()
        project = Project(customer_id=customer.id, name=f"_deact2_p_{suffix}")
        db.session.add(project)
        db.session.flush()
        case_review = Case(
            project_id=project.id,
            title=f"_deact_review_{suffix}",
            application_no=f"CN98{suffix}",
            business_owner_id=staff.id,
        )
        case_done = Case(
            project_id=project.id,
            title=f"_deact_done_{suffix}",
            application_no=f"CN97{suffix}",
            business_owner_id=staff.id,
        )
        db.session.add_all([case_review, case_done])
        db.session.flush()
        db.session.add_all(
            [
                Task(case_id=case_review.id, phase_status=TaskPhase.PENDING_REVIEW, assignee_id=staff.id),
                Task(case_id=case_done.id, phase_status=TaskPhase.COMPLETED, assignee_id=staff.id),
            ]
        )
        db.session.commit()
        staff_id = staff.id
        admin_name = admin.username
        review_case_id = case_review.id
        done_case_id = case_done.id
    ...
    with app.app_context():
        review_case = db.session.get(Case, review_case_id)
        done_case = db.session.get(Case, done_case_id)
        assert review_case.business_owner_id == staff_id
        assert done_case.business_owner_id == staff_id
        assert review_case.task.assignee_id == staff_id
        assert review_case.task.phase_status == TaskPhase.PENDING_REVIEW
        assert done_case.task.assignee_id == staff_id
        assert done_case.task.phase_status == TaskPhase.COMPLETED


def test_reactivate_staff_account_restores_login_and_assignment_pool():
    app = create_app()
    suffix = uuid4().hex[:8]
    with app.app_context():
        admin = User(username=f"_admin_react_{suffix}", role="admin")
        admin.set_password("secret")
        staff = User(username=f"_staff_react_{suffix}", role="staff", is_active=False)
        staff.set_password("secret")
        db.session.add_all([admin, staff])
        db.session.commit()
        admin_name = admin.username
    ...
    assert staff_name.encode("utf-8") not in page.data

    r_react = client.post(
        "/admin/accounts",
        data={"form_action": "reactivate_account", "user_id": str(staff_id)},
        follow_redirects=True,
    )
    ...
    r_board = client.get("/admin/task-board")
    assert f'value="{staff_id}"'.encode("utf-8") in r_board.data
    ...
    assert "员工工作台".encode("utf-8") in r_login.data
...
def test_admin_case_edit_can_update_case_and_task_phase():
    ...
    with app.app_context():
        ...
        case = Case(
            ...
            actual_return_at=datetime(2031, 5, 6, 7, 30, tzinfo=timezone.utc),
        )
        db.session.add(case)
        db.session.flush()
        task = Task(case_id=case.id, phase_status=TaskPhase.IN_PROGRESS)
    ...
    with app.app_context():
        ...
        assert edited_case.actual_return_at.replace(tzinfo=timezone.utc) == datetime(
            2031, 5, 6, 7, 30, tzinfo=timezone.utc
        )


def test_completing_case_uses_latest_approved_submit_for_review_time():
    app = create_app()
    suffix = uuid4().hex[:8]
    with app.app_context():
        admin = User(username=f"_complete_admin_{suffix}", role="admin")
        admin.set_password("secret")
        staff = User(username=f"_complete_staff_{suffix}", role="staff")
        staff.set_password("secret")
        customer = Customer(kind=CustomerKind.COMPANY, name=f"_complete_customer_{suffix}")
        db.session.add_all([admin, staff, customer])
        db.session.flush()
        project = Project(customer_id=customer.id, name=f"_complete_project_{suffix}")
        db.session.add(project)
        db.session.flush()
        case = Case(
            project_id=project.id,
            title=f"_complete_case_{suffix}",
            application_no=f"CNCOMPLETE{suffix}",
            business_owner_id=staff.id,
        )
        db.session.add(case)
        db.session.flush()
        task = Task(case_id=case.id, phase_status=TaskPhase.PENDING_SUBMIT, assignee_id=staff.id)
        first_upload_at = datetime(2026, 1, 2, 1, tzinfo=timezone.utc)
        selected_upload_at = datetime(2026, 1, 4, 3, tzinfo=timezone.utc)
        selected_submit_at = datetime(2026, 1, 4, 8, tzinfo=timezone.utc)
        after_approval_at = datetime(2026, 1, 6, 5, tzinfo=timezone.utc)
        db.session.add_all(
            [
                task,
                CaseMaterial(
                    ...
                    original_name="初稿.docx",
                    stored_name=f"first_{suffix}.docx",
                    created_at=first_upload_at,
                ),
                CaseMaterial(
                    case_id=case.id,
                    uploaded_by_id=staff.id,
                    original_name="定稿.docx",
                    stored_name=f"selected_{suffix}.docx",
                    created_at=selected_upload_at,
                ),
                CaseMaterial(
                    case_id=case.id,
                    uploaded_by_id=staff.id,
                    original_name="审核后文件.docx",
                    stored_name=f"after_{suffix}.docx",
                    created_at=after_approval_at,
                ),
                CaseReviewLog(
                    case_id=case.id,
                ...
                CaseReviewLog(
                    ...
                    action="approve",
                    created_at=datetime(2026, 1, 3, 2, tzinfo=timezone.utc),
                ),
                CaseReviewLog(
                    case_id=case.id,
                    operator_id=staff.id,
                    action="submit_for_review",
                    created_at=selected_submit_at,
                ),
                CaseReviewLog(
                    case_id=case.id,
                    operator_id=admin.id,
                    action="approve",
                    created_at=datetime(2026, 1, 5, 4, tzinfo=timezone.utc),
                ),
            ]
        )
        db.session.commit()
    ...
    assert response.status_code == 200
    with app.app_context():
        completed_case = db.session.get(Case, case_id)
        assert completed_case.task.phase_status == TaskPhase.COMPLETED
        assert completed_case.actual_return_at.replace(tzinfo=timezone.utc) == selected_submit_at

        # 启动补算不得覆盖人工已填写的实际返稿时间
        kept_at = datetime(2025, 12, 1, tzinfo=timezone.utc)
        completed_case.actual_return_at = kept_at
        db.session.commit()
        assert backfill_completed_case_actual_return_at() == 0
        db.session.commit()
        assert completed_case.actual_return_at.replace(tzinfo=timezone.utc) == kept_at


def test_actual_return_at_uses_submit_after_last_reject_before_approval():
    app = create_app()
    suffix = uuid4().hex[:8]
    with app.app_context():
        admin = User(username=f"_return_admin_{suffix}", role="admin")
        admin.set_password("secret")
        staff = User(username=f"_return_staff_{suffix}", role="staff")
        staff.set_password("secret")
        db.session.add_all([admin, staff])
        db.session.flush()
        customer = Customer(kind=CustomerKind.COMPANY, name=f"_return_customer_{suffix}")
        db.session.add(customer)
        db.session.flush()
        project = Project(customer_id=customer.id, name=f"_return_project_{suffix}")
        db.session.add(project)
        db.session.flush()
        case = Case(
            project_id=project.id,
            title=f"_return_case_{suffix}",
            application_no=f"CNRETURN{suffix}",
            business_owner_id=staff.id,
        )
        db.session.add(case)
        db.session.flush()
        first_submit_at = datetime(2026, 2, 1, 9, tzinfo=timezone.utc)
        resubmit_at = datetime(2026, 2, 3, 11, tzinfo=timezone.utc)
        db.session.add_all(
            [
                Task(case_id=case.id, phase_status=TaskPhase.PENDING_SUBMIT, assignee_id=staff.id),
                CaseReviewLog(
                    case_id=case.id,
                    operator_id=staff.id,
                    action="submit_for_review",
                    created_at=first_submit_at,
                ),
                CaseReviewLog(
                    case_id=case.id,
                    operator_id=admin.id,
                    action="reject",
                    created_at=datetime(2026, 2, 2, 10, tzinfo=timezone.utc),
                ),
                CaseReviewLog(
                    case_id=case.id,
                    operator_id=staff.id,
                    action="submit_for_review",
                    created_at=resubmit_at,
                ),
                CaseReviewLog(
                    case_id=case.id,
                    operator_id=admin.id,
                    action="approve",
                    created_at=datetime(2026, 2, 4, 15, tzinfo=timezone.utc),
                ),
            ]
        )
        db.session.commit()
        assert set_actual_return_at_from_latest_approved_writing(case).replace(
            tzinfo=timezone.utc
        ) == resubmit_at


def test_actual_return_at_uses_latest_staff_upload_when_completed_without_review():
    app = create_app()
    suffix = uuid4().hex[:8]
    with app.app_context():
        staff = User(username=f"_legacy_staff_{suffix}", role="staff")
        staff.set_password("secret")
        db.session.add(staff)
        db.session.flush()
        customer = Customer(kind=CustomerKind.COMPANY, name=f"_legacy_customer_{suffix}")
        db.session.add(customer)
        db.session.flush()
        project = Project(customer_id=customer.id, name=f"_legacy_project_{suffix}")
        db.session.add(project)
        db.session.flush()
        case = Case(
            project_id=project.id,
            title=f"_legacy_case_{suffix}",
            application_no=f"CNLEGACY{suffix}",
            business_owner_id=staff.id,
            actual_return_at=None,
        )
        db.session.add(case)
        db.session.flush()
        first_upload_at = datetime(2026, 3, 1, 9, tzinfo=timezone.utc)
        latest_upload_at = datetime(2026, 3, 5, 16, tzinfo=timezone.utc)
        db.session.add_all(
            [
                Task(case_id=case.id, phase_status=TaskPhase.COMPLETED, assignee_id=staff.id),
                CaseMaterial(
                    case_id=case.id,
                    uploaded_by_id=staff.id,
                    original_name="初稿.docx",
                    stored_name=f"legacy_first_{suffix}.docx",
                    created_at=first_upload_at,
                ),
                CaseMaterial(
                    case_id=case.id,
                    uploaded_by_id=staff.id,
                    original_name="终稿.docx",
                    stored_name=f"legacy_last_{suffix}.docx",
                    created_at=latest_upload_at,
                ),
            ]
        )
        db.session.commit()
        case_id = case.id
        assert case.actual_return_at is None

        runner = app.test_cli_runner()
        dry_run = runner.invoke(args=["backfill-actual-return-at"])
        assert dry_run.exit_code == 0
        assert "dry-run" in dry_run.output
        db.session.expire_all()
        assert db.session.get(Case, case_id).actual_return_at is None

        applied = runner.invoke(args=["backfill-actual-return-at", "--apply"])
        assert applied.exit_code == 0
        db.session.expire_all()
        filled = db.session.get(Case, case_id)
        assert filled.actual_return_at is not None
        assert filled.actual_return_at.replace(tzinfo=timezone.utc) == latest_upload_at
        # 无审核、有材料：不必再有审核记录
        assert CaseReviewLog.query.filter_by(case_id=case_id, action="approve").count() == 0
...
def test_task_board_admin_and_staff_use_unified_list_scope():
    ...
    with app.app_context():
        assert db.session.get(Task, task_bad_pending_id).phase_status == TaskPhase.PENDING_ASSIGNMENT
    ...
    assert f"_board_bad_pending_{suffix}".encode("utf-8") not in r_staff.data
    ...
    assert f"_board_bad_pending_{suffix}".encode("utf-8") not in r_case_list.data
...
def test_staff_upload_then_submit_for_review():
    ...
    with app.app_context():
        ...
        assert (
            CaseReviewLog.query.filter_by(case_id=case_id, action="submit_for_review").count()
            == 1
        )
...
def test_staff_uploaded_material_filename_visible_to_staff_and_admin():
    app = create_app()
    suffix = uuid4().hex[:8]
    ...
    assert staff_filename.encode("utf-8") in writing_section
    assert "定稿".encode("utf-8") in writing_section
    ...
    assert staff_filename.encode("utf-8") in materials_section
    assert "定稿".encode("utf-8") in materials_section
...
def test_case_material_download_creates_audit_logs():
    ...
    r_staff_detail = client.get(f"/staff/case-detail/{case_id}")
    assert r_staff_detail.status_code == 200
    assert "材料下载留痕".encode("utf-8") in r_staff_detail.data
...
def test_case_material_download_logs_filter_and_pagination():
    ...
    download_section = r_staff_only.data.split("材料下载留痕".encode("utf-8"), 1)[1]
